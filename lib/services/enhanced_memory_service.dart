...
class EnhancedMemoryService {
  ...
  EnhancedMemoryService({required String groqApiKey}) : _groqApiKey = groqApiKey;
    ...
    try {
      ...
      for (var memory in extractedMemories) {
        ...
        await _logBuddyActivity(
          ...
          title: memory['content'] != null && memory['content'].toString().length > 30 
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'messages': [
            {
              'role': 'system',
              'content': '''You are a memory extraction system for an elderly care app. Analyze conversations and extract important information with special attention to:
...
Extract important memories with emphasis on emotional state and any concerning patterns.'''
  ...
  Future<String> loadUserMemoryContext(String userId) async {
    try {
      ...
      String context = "Here is what you know about this user from previous conversations:\n";
      for (var memory in memories) {
        context += "- ${memory['content']} (Related to: ${memory['category']}, Sentiment: ${memory['sentiment']})\n";
      }
      return context;
    } catch (e) {
  ...
  Future<List<String>> getPersonalizedQuestions(String userId) async {
    try {
      ...
      if (snapshot.docs.isEmpty) {
        return ["How are you feeling today?", "What's on your mind?", "Is there anything you'd like to share?"];
      }

      final memoriesText = snapshot.docs.map((doc) {
        final data = doc.data();
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'messages': [
            {
              'role': 'system',
              'content': 'You are a caring companion assistant. Based on these user memories, generate 3 thoughtful, open-ended follow-up questions to ask the user. Return ONLY a JSON object: {"questions": ["q1", "q2", "q3"]}'
            },
            {
              'role': 'user',
    ...
    return ["How was your day?", "What have you been up to?", "How are you feeling?"];
  }

  // Build dynamic context for a specific message by finding relevant past memories
  Future<String> buildDynamicContext({required String userId, required String currentMessage}) async {
    try {
      ...
      String context = "Specifically, you remember these things that might be relevant:\n";
  ...
  Future<void> _checkAndNotifyCaretaker({
    ...
    try {
      ...
      final requiresNotification = memory['requiresCaretakerNotification'] == true ||
  ...
  Future<void> _createCaretakerNotification({
    ...
    try {
      ...
      if (memory['type'] == 'missing_someone' || memory['type'] == 'loneliness') {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'messages': [
            {
              'role': 'system',
              'content': '''You are a compassionate care assistant generating a weekly emotional wellness report for a caretaker. 
...
Provide a comprehensive yet concise summary that a caretaker can act upon.'''
  ...
  Future<List<Map<String, dynamic>>> getRelevantMemories({
    ...
    try {
      ...
      final keywords = messageLower.split(' ').where((word) => word.length > 3).toList();
      ...
      for (var doc in snapshot.docs) {
        ...
        for (var keyword in keywords) {
          if (content.contains(keyword) || 
              memoryKeywords.any((mk) => mk.toString().toLowerCase().contains(keyword))) {
