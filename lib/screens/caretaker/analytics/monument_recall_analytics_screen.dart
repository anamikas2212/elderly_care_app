...
class MonumentRecallAnalyticsScreen extends StatelessWidget {
  final String userId;
  const MonumentRecallAnalyticsScreen({super.key, required this.userId});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Monument Recall Analytics")),
      body: FutureBuilder<Map<String, dynamic>>(
        future: dataService.getMonumentRecallAnalytics(userId),
        builder: (context, snapshot) {
          ...
          final languageScore = stats['languageScore'] as int? ?? 0;
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(child: _buildSummaryCard("Memory", "$memoryScore", Colors.purple, improvement['memory'])),
                    const SizedBox(width: 10),
                    Expanded(child: _buildSummaryCard("Language", "$languageScore", Colors.pink, improvement['language'])),
                    const SizedBox(width: 10),
                    ...
                    child: LineChart(
                      LineChartData(
                        lineBarsData: [
                          ...
                          LineChartBarData(
                            spots: _buildCognitiveSpots(sessions, 'language'),
                            isCurved: true, color: Colors.pink, barWidth: 2.5,
                            dotData: FlDotData(show: sessions.length <= 10),
                            belowBarData: BarAreaData(show: true, color: Colors.pink.withValues(alpha: 0.08)),
                          ),
                        ],
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        _buildLegendItem("Language", Colors.pink),
                      ],
                    ),
                  ),
                ...
                Card(
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        _buildDetailRow("Memory Score", "$memoryScore / 100", improvement['memory']),
                        const Divider(),
                        _buildDetailRow("Language Score", "$languageScore / 100", improvement['language']),
                        const Divider(),
  ...
  Map<String, double?> _calculateImprovement(List<dynamic> sessions) {
    if (sessions.length < 4) return {'memory': null, 'language': null};
    int rc = (sessions.length * 0.3).ceil().clamp(1, 5);
    double rM = 0, pM = 0, rL = 0, pL = 0;
    for (int i = sessions.length - rc; i < sessions.length; i++) {
      ...
      rL += (c['language'] as num?)?.toDouble() ?? 0;
    }
    int ps = (sessions.length - rc - rc).clamp(0, sessions.length);
    int actualPc = (sessions.length - rc) - ps;
    if (actualPc <= 0) return {'memory': null, 'language': null};
    for (int i = ps; i < sessions.length - rc; i++) {
      final c = sessions[i]['cognitive_contributions'] as Map<String, dynamic>? ?? {};
      pM += (c['memory'] as num?)?.toDouble() ?? 0;
      pL += (c['language'] as num?)?.toDouble() ?? 0;
    }
    return {'memory': (rM / rc) - (pM / actualPc), 'language': (rL / rc) - (pL / actualPc)};
  }
